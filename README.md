...
**Why?** As of Flutter 3.7, you can call platform plugins from normal `Isolate`.  You should just use a normal `Isolate` instead of `dart_ui_isolate`. In the rare case you need to use both `dart:ui` & platform plugins from the same isolate, you should preferably use a normal isolate + `dart:ui` isolate and pass data between them, or use [flutter_isolate](https://pub.dev/packages/flutter_isolate).**, 
